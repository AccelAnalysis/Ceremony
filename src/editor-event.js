...
export function renderEventPanel(p,ctx){
 ...
 p.querySelector('#saveEvent').onclick=async()=>{await ctx.backend.updateEvent(ctx.eventId,{name:p.querySelector('#eventTitle').value.trim().slice(0,120),defaultDuration:clamp(num(p.querySelector('#defaultDuration').value,10),1,300),transitionSeconds:clamp(num(p.querySelector('#transitionSeconds').value,1.4),0,10),overlayOpacity:clamp(num(p.querySelector('#overlayOpacity').value,.58),0,1),crossfadeSeconds:clamp(num(p.querySelector('#crossfadeSeconds').value,2.5),.1,30),ceremonyStartAt:p.querySelector('#ceremonyStart').value?new Date(p.querySelector('#ceremonyStart').value):null,clockFormat:p.querySelector('#clockFormat').value,scheduleEnabled:p.querySelector('#scheduleEnabled').checked,catchUp:p.querySelector('#catchUp').checked,autoCueMusic:p.querySelector('#autoCueMusic').checked});toast('Event settings saved')};
