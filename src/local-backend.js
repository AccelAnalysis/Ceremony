...
export const localBackend={
 ...
 async claimSchedule(id,executionId,mutator){const db=load(),e=ensureEvent(db,id);if(e.executions[executionId])return false;const cur=e.runtime||{};const next=mutator(structuredClone(cur));if(!next)return false;e.executions[executionId]={executedAt:new Date().toISOString(),actorUid:'demo-user'};next.stateVersion=(cur.stateVersion||0)+1;next.updatedAt=new Date().toISOString();next.updatedBy='demo-user';if(next.currentSlideId!==cur.currentSlideId||(!cur.playing&&next.playing))next.slideStartedAt=new Date().toISOString();e.runtime=next;save(db);return true}
};
