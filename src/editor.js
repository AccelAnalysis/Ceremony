...
const services=await getBackend();
...
const nav=document.querySelector('#editorNav');
...
nav.onclick=e=>{
  ...
  panels.forEach(p=>p.classList.toggle('hidden',p.dataset.panel!==b.dataset.panel));
  render(b.dataset.panel);
};

backend.subscribeEvent(eventId,v=>{state.event=v;document.querySelector('#eventName').textContent=v?v.name:'Event not found';renderActive()});
for(const name of ['slides','categories','graduates','awards','tracks','schedule']){
  backend.subscribeCollection(eventId,name,v=>{state[name]=v;renderActive()});
}
backend.subscribeRuntime(eventId,v=>{state.runtime=v;renderActive()});

function active(){const p=panels.find(x=>!x.classList.contains('hidden'));return p?p.dataset.panel:'event'}
function renderActive(){render(active())}
function render(kind){
  const p=panels.find(x=>x.dataset.panel===kind);
  if(!p)return;
  if(!state.event&&kind!=='data'){
    p.innerHTML='<h2 class="section-title">Event not initialized</h2>'+notice('Create <b>'+esc(eventId)+'</b> from Setup / Data first.');
    return;
  }
  const renderers={event:renderEventPanel,slides:renderSlidesPanel,people:renderGraduatePanel,awards:renderAwardPanel,music:renderMusicPanel,schedule:renderSchedulePanel,visual:renderVisualPanel,data:renderDataPanel};
