...
let event=null,runtime=null,slides=[],graduates=[],awards=[],lastVersion=-1;

backend.subscribeEvent(eventId,v=>{event=v;if(!v){status.textContent='Event not found.';return}document.documentElement.style.setProperty('--overlay',String(v.overlayOpacity??.58));hud.setEvent(v);audio.setEvent(v);renderer.sync(slides,runtime,v)});
